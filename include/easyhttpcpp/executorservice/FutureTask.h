...
#ifndef EASYHTTPCPP_EXECUTORSERVICE_FUTURETASK_H_INCLUDED
#define EASYHTTPCPP_EXECUTORSERVICE_FUTURETASK_H_INCLUDED
...
class FutureTask : public easyhttpcpp::common::RefCountedRunnable, public easyhttpcpp::common::Future<Result> {
public:
    typedef Poco::AutoPtr<FutureTask<Result> > Ptr;

    // m_finishedEvent: autoReset of Poco::Event(bool autoReest) is false.
    // default constructor is autoReset = true and the event is automatically reset after a wait().
    // https://pocoproject.org/docs/Poco.Event.html
    FutureTask() : m_finishedEvent(false), m_cancelled(false), m_finished(false)
    {
    }
...
protected:
    virtual void run()
    {
        easyhttpcpp::common::RefCountedRunnable::Ptr guard(this, false); // ensure automatic release when done
        try {
...
#endif /* EASYHTTPCPP_EXECUTORSERVICE_FUTURETASK_H_INCLUDED */
